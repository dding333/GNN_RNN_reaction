...
class GEDPredDataset(dgl.data.DGLDataset):
    def __init__(self, args):
        ...
        self.graphs2 = []
        self.targets = []
        self.smiles1 = []  # SMILES strings for the first set of molecules
        self.smiles2 = []  # SMILES strings for the second set of molecules
    ...
    def save(self):
        ...
        # Save SMILES data as well
        with open(self.path + 'smiles1.pkl', 'wb') as f:
            pickle.dump(self.smiles1, f)
        with open(self.path + 'smiles2.pkl', 'wb') as f:
            pickle.dump(self.smiles2, f)
    ...
    def load(self):
        ...
        # Load SMILES data as well
        with open(self.path + 'smiles1.pkl', 'rb') as f:
            self.smiles1 = pickle.load(f)
        with open(self.path + 'smiles2.pkl', 'rb') as f:
            self.smiles2 = pickle.load(f)
    ...
    def process(self):
        ...
        with open(self.path + 'pairwise_ged.csv', 'w') as f:
            ...
            for g1, g2, s1, s2, ged in res:
                ...
                self.smiles1.append(s1)  # Save SMILES strings
    ...
    def has_cache(self):
        if os.path.exists(self.path + 'ged0.bin') and os.path.exists(self.path + 'ged1.bin') and os.path.exists(self.path + 'smiles1.pkl') and os.path.exists(self.path + 'smiles2.pkl'):
            print('cache found')
            return True
        else:
    ...
    def __getitem__(self, i):
        return self.graphs1[i], self.graphs2[i], self.targets[i], self.smiles1[i], self.smiles2[i]
